    ...
    public class VRTK_InteractableObject : MonoBehaviour
    {
        ...
        private void FixedUpdateTrackObject()
        {
            ...
            if (grabbedSnapHandle != null)
            {
                ...
                Debug.Log("TrackPoint.rotation = " + trackPoint.rotation + "grabbedSnapHandle.rotation = " + transform.rotation);
                //desiredOrientation = trackPoint.rotation  * Quaternion.Inverse(grabbedSnapHandle.rotation) * transform.rotation; 
                //Debug.Log("grappedSnapHandle");
            }
            else
            {
                ...
                Debug.Log("TrackPoint.rotation = "+ trackPoint.rotation + "tranfrom.rotation = " + transform.rotation);
                //desiredOrientation = trackPoint.rotation;
            }

            //Debug.Log(trackPoint.gameObject.name);
            ...
            if (angle != 0)
            {
                ...
                //rb.angularVelocity = angularTarget;//Vector3.MoveTowards(rb.angularVelocity, angularTarget, maxDistanceDelta);
                
                rb.angularVelocity = Vector3.RotateTowards(rb.angularVelocity, angularTarget, maxDistanceDelta, maxDistanceDelta); //Vector3.MoveTowards(rb.angularVelocity, angularTarget, maxDistanceDelta);
                //float distance = positionDelta.magnitude;
                //if (distance < 0.01f)
                   // rb.MoveRotation(desiredOrientation); // snap to the hands orientation
